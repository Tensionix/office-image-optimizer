...
title Audion Office Image Optimizer - Build Portable Env
...
set "BOOTSTRAP_PACKAGES=setuptools wheel packaging"
...
echo ======================================================================
echo   AUDION OFFICE IMAGE OPTIMIZER - BUILD PORTABLE ENV
echo ======================================================================
echo Root:       %ROOT%
...
echo [6/8] Installing bootstrap build packages...
"%RUNTIME_DIR%\python.exe" -m pip download --disable-pip-version-check --timeout 120 --retries 12 -d "%WHEELHOUSE_DIR%" %BOOTSTRAP_PACKAGES%
if errorlevel 1 goto ERR_BOOTSTRAP_WHEELS
"%RUNTIME_DIR%\python.exe" -m pip install --disable-pip-version-check --no-index --find-links="%WHEELHOUSE_DIR%" %BOOTSTRAP_PACKAGES%
if errorlevel 1 goto ERR_BOOTSTRAP_INSTALL
...
break > "%TMP_FILE%"
findstr /x /c:"..\system_core" "%PTH_FILE%" >nul 2>nul
if errorlevel 1 >>"%TMP_FILE%" echo ..\system_core
...
:ERR_BOOTSTRAP_WHEELS
echo [ERROR] Failed to download bootstrap build packages.
if not defined AUDION_NO_PAUSE pause
exit /b 1

:ERR_BOOTSTRAP_INSTALL
echo [ERROR] Failed to install bootstrap build packages.
if not defined AUDION_NO_PAUSE pause
exit /b 1
