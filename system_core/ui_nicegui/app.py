...
LABELS = {
    "ru": {
        ...
        "folder_paths": "Папки операции",
        "advanced": "Дополнительно",
        "actions": "Действия",
        "section_advanced": "Дополнительно",
        "section_compression": "Сжатие",
        ...
        "section_size_mode": "Выбор размера",
        "section_expert_resolution": "Для опытных пользователей",
        "section_resolution": "Разрешение",
        "section_run": "Запуск",
        ...
        "custom_resolution_required": "Необходимо ввести не менее 400x400 px",
    ...
    "en": {
        ...
        "folder_paths": "Operation folders",
        "advanced": "Advanced",
        "actions": "Actions",
        ...
        "section_size_mode": "Size Mode",
        "section_expert_resolution": "Experienced Users",
        ...
        "custom_resolution_required": "Enter at least 400x400 px.",
...
initial_source_path, initial_target_path = load_workspace_route_settings()
...
state: dict[str, Any] = {
    ...
    "destination_path": initial_target_path,
}
...
def add_log(message: str) -> None:
    ...
    state["lines"] = state["lines"][-TERMINAL_LOG_LINE_LIMIT:]
    state["line_seq"] = int(state["line_seq"]) + 1
    state["log_version"] = int(state["log_version"]) + 1


def reset_terminal_lines() -> None:
    state["lines"] = []
    state["line_seq"] = 0
    state["terminal_generation"] = int(state["terminal_generation"]) + 1
    state["log_version"] = int(state["log_version"]) + 1
...
def terminal_lines_html(lines: Iterable[str], leading_newline: bool = False) -> str:
    chunks = [f'<span class="audion-terminal-line">{ansi_to_html(str(line))}</span>' for line in lines]
    return "".join(chunks)


def terminal_html(lines: Iterable[str] | None = None) -> str:
    return terminal_lines_html(state["lines"] if lines is None else lines)
...
def pick_files(_title: str = "Add files to input") -> list[Path]:
...
def copy_item_to_input(source: Path) -> Path:
    input_root = current_source_path()
    ...
    target = unique_target(input_root / source.name)
    if source.is_dir():
...
def operation_with_field_values(operation: Operation) -> Operation:
...
def operation_click_handler(operation: Operation):
    async def handler() -> None:
        await start_operation(operation_with_field_values(operation))
...
def toggle_terminal_pin() -> None:
    ...
    if len(pinned) == len(cache["pinned"]):
...
async def pick_terminal_location(kind: str) -> None:
    try:
        if kind == "file":
            picked = await run.io_bound(pick_files, tr("pick_file"))
        else:
...
def field_control_tooltip(field: dict[str, Any]) -> str:
    ...
    The hint carries the explanation. The label is the fallback so a narrow
    control still tells the user what it is instead of staying mute.
    """
...
def field_condition_matches(condition: Any) -> bool:
    ...
    if not key:
        return False
    value = state.setdefault("field_values", {}).get(key, "")
...
def field_is_disabled(field: dict[str, Any]) -> bool:
    ...
    if disabled_when is not None and field_condition_matches(disabled_when):
        return True
    enabled_when = field.get("enabled_when")
    if enabled_when is not None and not field_condition_matches(enabled_when):
        return True
    return bool(field.get("disabled", False))


def field_is_visible(field: dict[str, Any]) -> bool:
    hidden_when = field.get("hidden_when")
    if hidden_when is not None and field_condition_matches(hidden_when):
        return False
    visible_when = field.get("visible_when")
    if visible_when is not None and not field_condition_matches(visible_when):
        return False
    return bool(field.get("visible", True))


def props_with_disabled(props: str, disabled: bool) -> str:
    return f"{props} disable" if disabled else props
...
def path_picker_kind(field: dict[str, Any]) -> str:
    picker = str(field.get("picker") or field.get("path_picker") or field_kind(field)).strip().lower()
    return "file" if picker in {"file", "files", "open_file"} else "folder"


async def pick_field_path(field: dict[str, Any], control: Any) -> None:
    key = field_id(field)
    if not key:
        return
    try:
        if path_picker_kind(field) == "file":
            picked = await run.io_bound(pick_files, field_label(field))
        else:
            picked = await run.io_bound(
                pick_folder,
                field_label(field),
                bool(field.get("show_new_folder_button", field.get("allow_new_folder", False))),
            )
    except Exception as exc:
        safe_notify(str(exc), "negative")
        return
    if not picked:
        safe_notify(tr("picker_cancelled"), "warning")
        return
    selected = str(picked[0])
    set_field_value(key, selected)
    control.set_value(selected)


def field_path_click_handler(field: dict[str, Any], control: Any):
    async def handler() -> None:
        await pick_field_path(field, control)
...
def render_field(field: dict[str, Any]) -> None:
    ...
    refresh_on_change = bool(field.get("refresh_on_change", False))

    disabled_class = " audion-field-disabled" if disabled else ""
    field_container = ui.element("div").classes(f"{field_container_classes(field)}{disabled_class}")
    ...
    with field_container:
        ...
        if kind in {"select", "choice", "format"}:
            select = ui.select(
                ...
                label=label,
                value=value,
                on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, event.value, item_refresh),
            )
            ...
            select.props(props_with_disabled(props, disabled)).classes("audion-select w-full")
        ...
        if kind in {"radio", "radiobuttons", "radio-buttons"}:
            ...
            ui.radio(
                ...
                value=value,
                on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, event.value, item_refresh),
            ).props(props_with_disabled("dense inline", disabled)).classes("audion-choice-row")
            if dynamic_option_source(field):
                ui.button(
                    tr("refresh_options"),
                    on_click=refresh_options_click_handler(field),
        ...
        if kind in {"number", "int", "integer", "float"}:
            number_input = ui.number(
                ...
                on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, event.value, item_refresh),
            ).props(props_with_disabled("dense outlined", disabled)).classes("audion-number w-full")
            with number_input.add_slot("append"):
                with ui.element("div").classes("audion-number-spinner"):
                    ...
                    ).props(props_with_disabled("dense flat round tabindex=-1", disabled)).classes("audion-number-spin-button")
                    ui.button(
        ...
        if kind in {"checkbox", "bool", "boolean", "toggle"}:
            ui.checkbox(
                label,
                value=bool(value),
                on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, bool(event.value), item_refresh),
            ).props(props_with_disabled("dense", disabled)).classes("audion-single-checkbox")
            if hint:
                ui.label(hint).classes("audion-field-hint")
            return
        ...
        if kind in {"path", "folder", "file"}:
            button_label = tr("pick_file") if path_picker_kind(field) == "file" else tr("pick_folder")
            button_icon = "insert_drive_file" if path_picker_kind(field) == "file" else "folder_open"
            with ui.row().classes("w-full items-start gap-2"):
                path_input = ui.input(
                    ...
                    on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, event.value, item_refresh),
                ).props(props_with_disabled("dense outlined", disabled)).classes("min-w-0 flex-1")
                ui.button(
                    button_label,
                    icon=button_icon,
                    on_click=field_path_click_handler(field, path_input),
                ).props(props_with_disabled("dense flat no-wrap", disabled)).classes("audion-action w-36 rounded-lg")
            if hint:
                ui.label(hint).classes("audion-field-hint")
            return
        ...
        ui.input(
            ...
            placeholder=str(field.get("placeholder", "")),
            on_change=lambda event, item_key=key, item_refresh=refresh_on_change: set_field_value(item_key, event.value, item_refresh),
        ).props(props_with_disabled("dense outlined", disabled)).classes("w-full")
        if hint:
            ui.label(hint).classes("audion-field-hint")
...
def validate_pending_fields(node: CommandNode) -> bool:
    values = state.setdefault("field_values", {})
    field_keys = {field_id(field) for field in node.fields}
    has_custom_resolution = {"resolution_mode", "custom_width", "custom_height"}.issubset(field_keys)
    if has_custom_resolution and str(values.get("resolution_mode") or "preset").strip().lower() == "custom":
        try:
            width = int(values.get("custom_width") or 0)
            height = int(values.get("custom_height") or 0)
        except (TypeError, ValueError):
            width = 0
            height = 0
        if width < 400 or height < 400:
            safe_notify(tr("custom_resolution_required"), "warning")
            return False
    for field in node.fields:
...
def command_visible_fields(fields: tuple[dict[str, Any], ...] | list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [field for field in fields if not is_module_path_field(field)]
...
def io_fields_from_fields(fields: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    return [field for field in fields if is_module_path_field(field)]


def module_path_fields(node: CommandNode | None) -> list[dict[str, Any]]:
    if node is None:
        return []
    return io_fields_from_fields(node.fields)


def global_io_fields() -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    seen: set[str] = set()
    for node in manifest.operation_groups:
        for field in module_path_fields(node):
            key = field_id(field)
            if key and key not in seen:
                result.append(field)
                seen.add(key)
    for operation in [*manifest.operations, *manifest.maintenance_operations]:
        for field in io_fields_from_fields(operation.fields):
            key = field_id(field)
            if key and key not in seen:
                result.append(field)
                seen.add(key)
...
def command_tree() -> None:
    ...
    if pending is not None:
        if pending.fields:
            ...
            primary_fields = [field for field in primary_fields if not is_module_path_field(field)]
            advanced_fields = [field for field in advanced_fields if not is_module_path_field(field)]
...
def terminal_command_bar() -> None:
    ...
    with ui.column().classes("audion-terminal-command w-full gap-1"):
        with ui.row().classes("w-full items-center gap-2"):
            ...
            shell_select.props("dense outlined").classes("audion-terminal-shell")

            command_select = ui.select(
                options=terminal_command_options(),
                label=tr("terminal_command"),
                value=str(state.get("terminal_command") or "") or None,
                on_change=lambda event: set_terminal_command(event.value),
            )
            command_select.props(
                "dense outlined use-input input-debounce=0 fill-input new-value-mode=add-unique "
                "popup-content-class=audion-select-popup"
            ).classes("audion-terminal-combo min-w-0 flex-1")
            command_select.on("update:input-value", lambda event: set_terminal_command(event_value(event)))
            command_select.on("keydown.enter", terminal_enter_handler)
            ...
            ui.button(
                tr("unpin_command") if terminal_command_is_pinned() else tr("pin_command"),
                on_click=toggle_terminal_pin,
            ).props("dense flat no-wrap").classes("audion-action audion-terminal-pin rounded-lg")
            ui.button(
                tr("clear_terminal_history"),
                on_click=clear_terminal_history,
            ).props("dense flat no-wrap").classes("audion-action audion-terminal-clear rounded-lg")

        with ui.row().classes("w-full items-center gap-2"):
            ui.input(
                label=tr("terminal_location"),
...
def build_ui() -> None:
    ...
    last_terminal_generation = {"value": -1}
    last_line_seq = {"value": 0}

    refresh_timer: Any | None = None

    def append_terminal_html(chunk_html: str, *, reset: bool = False) -> None:
        chunk_json = json.dumps(chunk_html, ensure_ascii=False)
        reset_json = "true" if reset else "false"
        ui.run_javascript(
            f"""
            requestAnimationFrame(() => {{
              const chunk = {chunk_json};
              const reset = {reset_json};
              const lineLimit = {TERMINAL_LOG_LINE_LIMIT};
              ...
              document.querySelectorAll('.audion-terminal').forEach((el) => {{
                ...
                const shouldScroll = !hasSelection && wasAtBottom;
                if (reset) {{
                  el.replaceChildren();
                }}
                if (chunk) {{
                ...
                const lines = Array.from(el.children).filter((child) =>
                  child.classList && child.classList.contains('audion-terminal-line')
                );
                const removeCount = Math.max(0, lines.length - lineLimit);
    ...
    def refresh() -> None:
        ...
        try:
            ...
            if log_version != last_log_version["value"]:
                ...
                generation = int(state["terminal_generation"])
                line_seq = int(state["line_seq"])
                lines = [str(line) for line in state["lines"]]
                reset = generation != last_terminal_generation["value"] or line_seq < last_line_seq["value"]
                if reset:
                    chunk_lines = lines
                    last_terminal_generation["value"] = generation
                else:
                    new_count = line_seq - last_line_seq["value"]
                    chunk_lines = lines[-new_count:] if new_count > 0 and new_count <= len(lines) else []
                    if new_count > len(lines):
                        reset = True
                        chunk_lines = lines
                last_line_seq["value"] = line_seq
                if reset or chunk_lines:
                    append_terminal_html(terminal_lines_html(chunk_lines), reset=reset)
