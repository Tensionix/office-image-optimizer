...
title Audion Office Image Optimizer - Launcher
...
set "DOC_FILE=%RUNTIME_DIR%\project_docs_en.txt"
set "DOC_RES=%RUNTIME_DIR%\project_docs_en_res.txt"

del /q "%RUNTIME_DIR%\project_menu_en*.txt" "%RUNTIME_DIR%\project_docs_en*.txt" "%RUNTIME_DIR%\opt_mode*.txt" "%RUNTIME_DIR%\opt_docs*.txt" >nul 2>nul
...
cls
echo ======================================================================
echo   AUDION OFFICE IMAGE OPTIMIZER - LAUNCHER
echo ======================================================================
echo Root:      %BASE_DIR%
...
"%FZF_CMD%" --prompt="audion@image-optimizer [PROJECT-EN] > " --pointer=">" --header="Choose mode. Quick modes first, service actions below." --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%MENU_FILE%" > "%RES_FILE%"
...
if "%DOC_SIZE%"=="0" (
  ...
  echo [ERROR] No DOCX or PPTX files were found in input.
  if not defined AUDION_NO_PAUSE pause >nul
  goto :eof
)
sort "%DOC_FILE%" /o "%DOC_FILE%.sorted" >nul
"%FZF_CMD%" --prompt="Document > " --pointer=">" --header="Select DOCX/PPTX from input" --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%DOC_FILE%.sorted" > "%DOC_RES%"
...
set "TARGET="
echo.
set /p TARGET=Enter DOCX/PPTX file path: 
goto :eof
