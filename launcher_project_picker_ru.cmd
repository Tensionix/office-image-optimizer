...
title Audion Office Image Optimizer - Русский Picker
...
set "MENU_FILE=%RUNTIME_DIR%\project_picker_menu_ru.txt"
set "RES_FILE=%RUNTIME_DIR%\project_picker_menu_ru_res.txt"
set "DOC_FILE=%RUNTIME_DIR%\project_picker_docs_ru.txt"
set "DOC_RES=%RUNTIME_DIR%\project_picker_docs_ru_res.txt"
set "PICK_RESULT=%RUNTIME_DIR%\project_picker_result_ru.txt"

del /q "%RUNTIME_DIR%\project_picker_menu_ru_*.txt" "%RUNTIME_DIR%\project_picker_docs_ru_*.txt" "%RUNTIME_DIR%\project_picker_result_ru_*.txt" >nul 2>nul
...
cls
echo ======================================================================
echo   AUDION OFFICE IMAGE OPTIMIZER - РУССКИЙ ПРОЕКТНЫЙ ЛАУНЧЕР (PICKER)
echo ======================================================================
...
"%FZF_CMD%" --prompt="audion@image-optimizer [PROJECT-RU+PICKER] > " --pointer=">" --header="Выберите режим. Затем можно выбрать файл из списка или открыть диалог." --layout=reverse --border=rounded --info=hidden --margin=1,2  < "%MENU_FILE%" > "%RES_FILE%"
...
set "TARGET="
type nul > "%DOC_FILE%"
echo ВЫБРАТЬ ФАЙЛ...>>"%DOC_FILE%"
if exist "%BASE_DIR%\input" for /r "%BASE_DIR%\input" %%F in (*.docx *.pptx) do @echo %%~fF>>"%DOC_FILE%"
sort "%DOC_FILE%" /o "%DOC_FILE%.sorted" >nul
"%FZF_CMD%" --prompt="Документ > " --pointer=">" --header="ВЫБРАТЬ ФАЙЛ... = открыть диалог выбора ^| или взять из input" --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%DOC_FILE%.sorted" > "%DOC_RES%"
del /q "%DOC_FILE%" "%DOC_FILE%.sorted" >nul 2>nul
set /p TARGET=<"%DOC_RES%"
del /q "%DOC_RES%" >nul 2>nul
if /I "%TARGET%"=="ВЫБРАТЬ ФАЙЛ..." call :PICK_BROWSE
goto :eof

:PICK_DOC_CMD
set "TARGET="
echo.
set /p TARGET=Введите путь или оставьте пусто для выбора файла: 
...
if not exist "%BROWSE_CMD%" (
  echo [ERROR] В system_core не найден Browse-Office-File.cmd.
  goto :eof
)
